 ...
 */
async function runPreSignInSignOut(opts: {
  livePreview: boolean;
  hasBearer: boolean;
  ...
  if (opts.livePreview) {
    if (opts.hasBearer) {
      try {
        await Promise.race([opts.requestSignOut(), new Promise((resolve) => window.setTimeout(resolve, 1500))]);
      } catch {
        /* preview token is cleared locally below */
      }
    }
    opts.clearToken();
    return;
...
async function runSignOut(opts: {
  livePreview: boolean;
  hasBearer: boolean;
  requestSignOut: () => Promise<unknown>;
  clearToken: () => void;
  ...
  if (opts.livePreview) {
    if (opts.hasBearer) {
      try { await opts.requestSignOut(); } catch { /* local preview state can still be cleared */ }
    }
    opts.clearToken();
    opts.redirect();
    return;
...
export function rememberAuthToken(token: string | null | undefined) {
...
export async function signOut(redirectTo = "/"): Promise<void> {
  try {
    const { setLocalMember } = await import("@/lib/app-core");
    setLocalMember(null);
  } catch {
    /* ignore */
  }

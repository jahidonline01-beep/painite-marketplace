 ...
 */
function memberFromMintedToken(token?: string): VerifiedUser | null {
  if (!token || !token.startsWith("pm3.")) return null;
  const phone = token.slice(4);
  if (!/^\d{1,11}$/.test(phone)) return null;
  return { id: `member-${phone}`, email: `${phone}@painite.app` };
}

export async function getSessionUser(
  bearerToken?: string,
): Promise<VerifiedUser | null> {
  const minted = memberFromMintedToken(bearerToken);

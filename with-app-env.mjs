...
function main(argv) {
  ...
  const child = spawn(command, args, {
    stdio: "inherit",
    env,
    // On Windows, npm-installed CLI entry points are `.cmd` files.
    // Running without a shell makes `vite` resolve on Unix but fail with
    // ENOENT on Windows. The shell is used only on Windows so all npm
    // scripts keep working cross-platform without changing their commands.
